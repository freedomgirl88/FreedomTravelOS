...
export default function Flight({ store }) {
  ...
  return <Page>
    ...
    <ModalSheet title={`Edit ${editing === "return" ? "Return" : "Outbound"} Flight`} open={Boolean(editing)} onClose={() => setEditing(null)}><div className="form-grid sheet-form">{(editing === "return" ? returnFields : outboundFields).map((key) => <EditableField key={key} label={key} value={flight?.[key] ?? ""} onChange={(v) => updater(key, v)} />)}</div></ModalSheet>
  </Page>;
}
