...
import { Plane, Hotel, Package, Wallet, MapPin, BookOpen, BellRing, Sparkles, Languages } from "lucide-react";
...
export default function Dashboard({ store, setActivePage }) {
  ...
  return <Page>
    <header className="app-header beta-header"><div><span className="eyebrow">Freedom Travel OS · Beta 1</span><h1>{trip.traveller}</h1></div><span className="status-chip">{days} days</span></header>
    ...
    <Card className="airport-alert-card" onClick={() => setActivePage("airport")}><BellRing size={22}/><div><span className="eyebrow">Return Flight Safety</span><strong>Leave {trip.returnFlight.leaveFrom} by {trip.returnFlight.leaveByTime} on 26 Aug</strong><p>Target {trip.returnFlight.departureAirport} at {trip.returnFlight.airportTargetTime} for {trip.returnFlight.flightNumber} at {trip.returnFlight.departureTime}.</p></div></Card>
    ...
    <SectionTitle title="Quick Actions" /><div className="quick-action-row"><button onClick={() => setActivePage("companion")}><Languages size={18}/> Korea Tools</button><button onClick={() => setActivePage("airport")}><BellRing size={18}/> Airport Plan</button><button onClick={() => setActivePage("assistant")}><BookOpen size={18}/> Assistant</button><button onClick={() => setActivePage("explore")}><MapPin size={18}/> Explore</button><button onClick={() => setActivePage("flight")}><Plane size={18}/> Flight</button><button onClick={() => setActivePage("budget")}><Wallet size={18}/> Budget</button></div>
