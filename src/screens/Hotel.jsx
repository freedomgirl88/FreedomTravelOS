...
export default function Hotel({ store }) {
  ...
  return <Page>
    ...
    <ModalSheet title="Edit Shilla Stay Details" open={open} onClose={() => setOpen(false)}><div className="form-grid sheet-form">{fields.map((key) => <EditableField key={key} label={key} value={h[key] ?? ""} onChange={(v) => updateHotelField(key, v)} />)}</div></ModalSheet>
  </Page>;
}
