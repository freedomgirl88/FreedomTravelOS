...
import { getLastBackupAt } from "../utils/dataProtection";
...
export default function TripHealth({ store, setActivePage }) {
  ...
  const backupAgeDays = lastBackup ? (Date.now() - new Date(lastBackup).getTime()) / 86400000 : Infinity;
  const backupScore = backupAgeDays <= 2 ? 100 : backupAgeDays <= 7 ? 60 : 0;
  ...
  const categories = [
    ...
    { label: "Explore plan started", score: exploreScore, weight: 5, action: "explore" },
    { label: "Recent data backup", score: backupScore, weight: 5, action: "settings" },
  ];
